...
export interface MEKernelLike extends Record<string, any> {
    ...
    tryEvaluateAssignExpression(evalScopePath: SemanticPath, expr: string): {
        ok: true;
    ...
    rebuildIndex(): void;
    resolveIndexPointerPath(path: SemanticPath, maxHops?: number): {
        path: SemanticPath;
        raw: any;
    };
    ...
    evaluateFilterClauseForScope(scope: SemanticPath, clause: {
        left: string;
        op: ">" | "<" | ">=" | "<=" | "==" | "!=";
        right: string;
    }): boolean;
    ...
    parseSelectorSegment(segment: string): {
        base: string;
        selector: string;
    } | null;
    parseSelectorKeys(selector: string): string[] | null;
    parseTransformSelector(selector: string): {
        varName: string;
        expr: string;
    } | null;
