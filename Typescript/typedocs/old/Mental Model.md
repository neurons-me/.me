...
`.me` is: A **semantic tree writer/reader** using paths like `wallet.income` With **two storage planes:**
...
**Root call ambiguity resolution** Inside `handleCall` when `path.length === 0`:

If you call `me("something")` and the string looks like:

- a dotted path `a.b`
- an operator-prefixed token `_`, `~`, `@`, `->`, `__`
- **OR** a single label `username` → it is treated as a `GET` = `readPath`, not a write.
...
`operators: Record` is the router.
...
**Golden rule:**`["_"]` = who can enter the room + makes the door stealth`["~"]` = which photo on the wall the story starts from, but you need the room first